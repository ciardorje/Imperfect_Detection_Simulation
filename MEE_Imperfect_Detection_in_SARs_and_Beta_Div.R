...
library(pacman)
p_load(dplyr, SpadeR, mobsim, nimble, data.table, tidyr, betapart, gridExtra, grid,
...
estimate_diversity <- function(x){
  ...
  for(i in 1:length(obs_data)){
    ...
    #Combine Results
    patchR <- rbind(NPE_results, HMSOM_R_results, observed_results)
    ...
    patchB <- rbind(Observed_B, HMSOM_B_Ests, NPE_B_Ests_Pooled, NPE_B_Ests_Single)
    ...
    HMSOM_SAR_data <- patchR[patchR$Estimator == 'HMSOM', c(2,6,7)]
    ...
    SARs <- list(
      ...
      HMSOM = lm(log(HMSOM_SAR_data$Estimate + 1) ~ log(HMSOM_SAR_data$Area)),
    ...
    #Collate results
    SAR_results <- as.data.frame(matrix(nrow = 7, ncol = 10))
    ...
    SAR_results$Non_NA[SAR_results$Estimator == 'iChao1.Single'] <- mean(!is.na(iChao1_Single_Data$SE.SD))
    SAR_results$Non_NA[SAR_results$Estimator == 'iChao1.Pooled'] <- mean(!is.na(iChao1_Pooled_Data$SE.SD))
    SAR_results$Non_NA[SAR_results$Estimator == 'Jack2.Single'] <- mean(!is.na(Jack2_Single_Data$SE.SD))
    SAR_results$Non_NA[SAR_results$Estimator == 'Jack2.Pooled'] <- mean(!is.na(Jack2_Pooled_Data$SE.SD))
    SAR_results$Non_NA[SAR_results$Estimator == 'ACE.Single'] <- mean(!is.na(ACE_Single_Data$SE.SD))
    SAR_results$Non_NA[SAR_results$Estimator == 'ACE.Pooled'] <- mean(!is.na(ACE_Pooled_Data$SE.SD))
    ...
    BModDat$SE.SD <- BModDat$SE.SD + 1e-3
    ...
    Bchao_pool_dat <- BModDat[BModDat$Estimator == 'iChao1' & 
                              ...
                              c('Area_Diff', 'Estimate', 'SE.SD')]
    Bhmsom_dat <- BModDat[BModDat$Estimator == 'HMSOM',
                              c('Area_Diff', 'Estimate', 'SE.SD')]
    ...
    #Collate results
    BMod_Results <- as.data.frame(matrix(nrow = 3, ncol = 10))
    ...
    #Calculate proportion of pairs where iChao could quantify uncertainty (i.e., 1 dissimilarity wasn't assigned instead)
    BMod_Results$Non_NA <- NA
    BMod_Results$Non_NA[BMod_Results$Estimator == 'iChao1.Single'] <- mean(!is.na(Bchao_single_dat$SE.SD))
    BMod_Results$Non_NA[BMod_Results$Estimator == 'iChao1.Pooled'] <- mean(!is.na(Bchao_pool_dat$SE.SD))
...
sim_ests <- foreach(i = 1:length(landscape_sims_parsed),
                     .options.snow = opts,
                     .inorder = F,
                     .packages = c('nimble', 'SpadeR', 'betapart', 'reshape2')
  ) %dopar% {
...
est_performance <- function(x){
  ...
  Incomplete_SARs <- SAREsts[!is.na(SAREsts$Non_NA) & SAREsts$Non_NA < 0.8,]
...
siteR <- siteR %>% ungroup() %>% 
  select(Patch, Area, True_R, Estimator, Samples, 
         Mean_Bias, Mean_Accuracy, Mean_Coverage, 
         Precision, Non_NA_Site, Sampling_Events,
...
estimators <- c('Observed', 'iChao1', 'HMSOM')
...
siteB <- siteB %>% 
  ...
  select(Site_1, Site_2, Estimator, Samples, True_B,  
         Mean_Bias, Mean_Accuracy, Mean_Coverage, Precision, Non_NA_Pair, 
...
b_estimators <- c('Observed', 'iChao1', 'HMSOM')
...
SARz <- SARz %>% 
  ...
  select(Estimator, z_Mean_Bias, z_Mean_Accuracy, z_Mean_Coverage, z_Precision,
         ID, Detection_Scenario, Non_NA_Repeats, Sampling_Events) %>%
  distinct()
...
SARov <- SARov %>%
  ungroup() %>% 
  select(Estimator, Mean_Model_Accuracy, ID, Detection_Scenario, Non_NA_Repeats, Sampling_Events) %>% 
  distinct()
...
REsts <- REsts %>% 
  ...
  select(Estimator, Samples, Mean_Coverage, Mean_Width_95,
         Non_NA_Site, ID, Sampling_Events, Detection_Scenario) %>% 
  distinct() %>% 
  filter(Estimator %in% c('iChao1', 'HMSOM'))  %>% 
  filter(Samples != 'Single' | is.na(Samples)) %>% 
  filter(Non_NA_Site >= 0.8)
REsts$Estimator <- factor(REsts$Estimator, levels = c('iChao1', 'HMSOM'))

r_uncert_plots <- plotty(REsts,
                         type = 'Uncertainty',
                         estimators = c('iChao1', 'HMSOM'),
                         labels = c('Chao', 'MSOM'),
...
BEsts <- BEsts %>% 
  ...
  select(Estimator, Samples, Mean_Coverage, Mean_Width95,
         Non_NA_Pair, ID, Sampling_Events, Detection_Scenario) %>% 
  distinct() %>% 
  filter(Estimator %in% c('iChao1', 'HMSOM'))  %>% 
  filter(Samples != 'Single' | is.na(Samples)) %>% 
  filter(Non_NA_Pair >= 0.8)
BEsts$Estimator <- factor(BEsts$Estimator, levels = c('iChao1', 'HMSOM'))
...
SAREsts <- separate(SAREsts, Estimator, into = c('Estimator', 'Samples'), sep = '\\.')
SAREsts <- SAREsts[SAREsts$Samples != 'Single' | is.na(SAREsts$Samples),]

SAREsts <- SAREsts %>% 
  ungroup() %>% 
  select(Estimator, Type, z_Mean_Coverage, z_Mean_Width95,
         c_Mean_Coverage, c_Mean_Width95, Non_NA_Repeats, ID, 
         Sampling_Events, Detection_Scenario) %>% 
  distinct() %>% 
  filter(Estimator %in% c('Observed', 'iChao1', 'HMSOM'))  %>% 
  filter(Type == 'LM') %>% 
  filter(Non_NA_Repeats >= 0.8)
SAREsts$Estimator <- factor(SAREsts$Estimator, 
                            levels = c('Observed', 'iChao1', 'HMSOM'))
...
BModEsts <- separate(BModEsts, Estimator, into = c('Estimator', 'Samples'), sep = '\\.')
BModEsts <- BModEsts[BModEsts$Samples != 'Single' | is.na(BModEsts$Samples),]

BModEsts <- BModEsts %>% 
  ungroup() %>% 
  select(Estimator, Type, b_Mean_Coverage, b_Mean_Width95,
         a_Mean_Coverage, a_Mean_Width95, Non_NA_Repeats, ID, 
         Sampling_Events, Detection_Scenario) %>% 
  distinct() %>% 
  filter(Estimator %in% c('Observed', 'iChao1', 'HMSOM'))  %>% 
  filter(Type == 'LM') %>% 
...
REsts <- REsts %>% 
  ...
  select(ID, Estimator, Patch, Area, Estimate, True_R, Samples, Repeat,
         Detection_Scenario, bArea_Shape, Sampling_Events, Non_NA_Site) %>% 
  distinct() %>% 
  filter(Estimator %in% c('Observed', 'iChao1', 'HMSOM'))  %>% 
  filter(Samples == 'Pooled' | is.na(Samples)) %>% 
  filter(Non_NA_Site >= 0.8 | Estimator == 'Observed') %>% 
  filter(Repeat == 1) %>% 
...
for(i in 1:9){
  ...
  ests$Estimator <- factor(ests$Estimator, levels = c('True', 'Observed', 'iChao1', 'MSOM'))
  sar_plots[[i]] <- 
...
chao_mod <- lm(Mean_Bias ~ mu_ip + sd_ip + Sampling_Events + log10(Area), 
                       data = REsts[REsts$Estimator == 'iChao1',])
...
chao_df$Method <- 'iChao1'
msom_df <- ggpredict(msom_mod, terms = 'Area [25:20000]')
msom_df$Method <- 'MSOM'

r_bias_df <- rbind(obs_df, chao_df, msom_df)
r_bias_df$Method <- factor(r_bias_df$Method, levels = c('Observed', 'iChao1', 'MSOM'))
...
b_chao_mod <- lm(Mean_Bias ~ mu_ip + bArea_Shape + sd_ip + Sampling_Events + log10(Area_Diff + 1e-5), 
                   data = BEsts[BEsts$Estimator == 'iChao1',])
